...
exports.getAllRewards = async (req, res) => {
  try {
    const rewards = await Reward.findAll({
      include: {
        model: Code,
        attributes: ['id', 'code', 'used', 'useDate', 'redeemDate'],
        include: {
          model: User,
          attributes: ['name']
        },
...
exports.updateReward = async (req, res) => {
  ...
  const codes = JSON.parse(req.body.codes);
  try {
    const reward = await Reward.update(update, { where: { id } });
    const listOfCodes = codes.map((code) => {
      if (code.rewardId) return code;
      return { ...code, rewardId: id };
    });
    Code.bulkCreate(listOfCodes, {
